...
use crate::steam_client::steam_app_list_vtable::STEAMAPPLIST_INTERFACE_VERSION;
use crate::steam_client::steam_app_list_wrapper::SteamAppList;
...
impl<'a> SteamClient<'a> {
    pub unsafe fn from_raw(ptr: *mut ISteamClient, callback_fn: Symbol<'a, SteamGetCallbackFn>, free_callback_fn: Symbol<'a, SteamFreeLastCallbackFn>) -> Self {
        Self {
    ...
    pub fn run_callbacks(&mut self, pipe: &HSteamPipe) -> Result<(), SteamError> {
        ...
        unsafe {
            ...
            if success {
                ...
                dev_println!("Callbacked: {}", message.id);
    ...
    pub fn get_isteam_app_list(
        &self,
        user: HSteamUser,
        pipe: HSteamPipe,
    ) -> Result<SteamAppList, SteamError> {
        unsafe {
            let version = STEAMAPPLIST_INTERFACE_VERSION.as_ptr() as *const c_char;

            let vtable = (*self.inner.ptr).vtable.as_ref().ok_or(SteamError::NullVtable)?;
            let app_list_ptr = (vtable.get_isteam_app_list)(self.inner.ptr, user, pipe, version);

            if app_list_ptr.is_null() {
                Err(SteamError::InterfaceCreationFailed("ISteamAppList".to_owned()))
            } else {
                Ok(SteamAppList::from_raw(app_list_ptr))
            }
        }
    }
